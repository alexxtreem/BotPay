...
def send_welcome(message):
    chat_id = message.chat.id
    if is_admin(chat_id):
        bot.reply_to(message, "Привет! Вы администратор клуба. Используйте команды /adduser для добавления нового пользователя, /pay для внесения членского взноса пользователю и /balance для проверки баланса пользователей.")
    else:
    ...
    if is_admin(chat_id):
        try:
            ...
            if user_id in users:
                ...
                save_users(users)
                bot.reply_to(message, f"Сумма в размере {payment_amount} успешно внесена пользователю с id {user_id}.")
            else:
...
def check_balance(message):
    ...
    if is_admin(chat_id):
        ...
        for user_id, user in users.items():
            response += f"{user.username}: {user.balance}\n"
